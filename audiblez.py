...
def main(kokoro, file_path, lang, voice, pick_manually, speed, providers):
    ...
    for text in texts:
        ...
        print(f'Chapter {i} read in {delta_seconds:.2f} seconds ({chars_per_sec:.0f}' +
...
def find_chapters(book, verbose=False):
    chapters = [c for c in book.get_items() if is_chapter(c)]
    if verbose:
        for item in book.get_items():
            if item.get_type() == ebooklib.ITEM_DOCUMENT:
                print(f"'{item.get_name()}'" + ', #' + str(len(item.get_body_content())))
                #print(f'{item.get_name()}'.ljust(60), str(len(item.get_body_content())).ljust(15),
                #    'X' if item in chapters else '-')
    if len(chapters) == 0:
...
def cli_main():
    if not Path('kokoro-v0_19.onnx').exists() or not Path('voices.json').exists():
        ...
        print('Error: kokoro-v0_19.onnx and voices.json must be in the current directory. ' +
            'Please download them with:')
        print('wget ' + kokoro_base + '/kokoro-v0_19.onnx')
        print('wget ' + kokoro_base + '/voices.json')
        sys.exit(1)
    ...
    parser.add_argument('-s', '--speed',
        default=1.0,
        help='Set speed from 0.5 to 2.0',
        type=float)
    ...
    main(kokoro, args.epub_file_path, args.lang, args.voice,
        args.pick, args.speed, args.providers)
